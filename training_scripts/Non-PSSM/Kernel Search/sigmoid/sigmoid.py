...
def matrix (wordlength,length,instance):
	...
	title = 'Learning Curves (SVM, sigmoid kernel, $\gamma=%.6f$)' %classifier.best_estimator_.gamma
	estimator = svm.SVC(kernel='sigmoid', gamma=classifier.best_estimator_.gamma)
	plot_learning_curve(estimator, title, X_train, y_train, cv=cv)
	plt.savefig('sigmoid-word-%04d.pdf' % instance)
